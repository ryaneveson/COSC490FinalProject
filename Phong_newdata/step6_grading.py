...
PATTERN_TYPE_PENALTIES = {
    ...
    "leet_speak": 8,
    "keyboard_pattern": 6,
    "special_digit": 0,  # Mixing specials + digits is strong, not weak
    "digit_only": 2,
...
# Common first names for name-based penalty (cracked passwords were disproportionately names)
COMMON_FIRST_NAMES = {
    ...
    "jordan", "albert", "willie", "alan", "eugene", "jesse", "austin", "gabriel",
    ...
    "alexis", "lori", "charlie", "sophie", "taylor", "logan", "luke", "dylan",
    "hunter", "mason", "ethan", "noah", "liam", "aiden", "jackson", "caden",
    "max", "sam", "alex", "mike", "chris", "matt", "nick", "dan", "tom", "ben",
    "jake", "joe", "steve", "dave", "bob", "jim", "tony", "rick", "bill", "jeff",
    "angel", "princess", "diamond", "summer", "bailey",
}
...
class PasswordGrader:
    ...
    def grade_password(self, password: str) -> Dict[str, Any]:
        ...
        # Short-password penalty: -10 for <6 chars, -5 for <8 chars
        short_penalty = 0
        if length < 6:
            short_penalty = 10
        elif length < 8:
        ...
        # Personal name penalty: -15 if password is/contains a common first name
        name_penalty = 0
        pwd_alpha = re.sub(r'[^a-zA-Z]', '', password).lower()
        if pwd_alpha in COMMON_FIRST_NAMES:
            name_penalty = 15

        # Final score calculation
        final_score = (base_predictability + complexity_bonus
                       - pattern_penalty - specific_penalty
                       - short_penalty - name_penalty)
        ...
        return {
            ...
            "short_password_penalty": short_penalty,
            "name_penalty": name_penalty,
            "complexity_bonus": complexity_bonus,
            "length": length,
            "breakdown": f"Base: {base_predictability:.1f} + Complexity: {complexity_bonus} - Pattern: {pattern_penalty} - Specific: {specific_penalty} - Short: {short_penalty} - Name: {name_penalty} = {final_score:.1f}"
        }
...
def sample_passwords_from_data(data_dir: Path, folders, sample_size: int = 20) -> List[str]:
    """Load ALL passwords from the 100k-most-used-passwords-NCSC.txt file (no sampling)"""
...
def main():
    ...
    # Load ALL real passwords from the new dataset file
    print("Loading ALL real passwords from new dataset file...")
    data_file = os.path.join(script_dir.parent, 'data', 'archive', '100k-most-used-passwords-NCSC.txt')
    real_passwords = []
    if not os.path.exists(data_file):
        print(f"Error: {data_file} not found")
    else:
        with open(data_file, 'r', encoding='utf-8', errors='replace') as f:
            for line in f:
                password = line.strip()
                if password and len(password) >= 4:
                    real_passwords.append((None, password))
    print(f"Loaded {len(real_passwords)} real passwords")
