...
class PasswordGeneralAnalysis:
    ...
    def load_passwords(self):
        """Load all passwords from the 100k-most-used-passwords-NCSC.txt file"""
        all_passwords = []
        if not os.path.exists(self.data_file):
            print(f"Error: {self.data_file} not found")
            return all_passwords
        print(f"Loading passwords from {self.data_file} ...")
        try:
            with open(self.data_file, 'r', encoding='utf-8', errors='ignore') as f:
                passwords = [line.strip() for line in f if line.strip()]
                all_passwords.extend(passwords)
                print(f"  - Loaded {len(passwords)} passwords from {os.path.basename(self.data_file)}")
    ...
    def _analyze_by_category(self, password_counter):
        """No-op: Category analysis not applicable for single file dataset."""
        print("Category analysis skipped: only one dataset file in use.")
...
def main():
    # Adjust path to new dataset file
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    data_file = os.path.join(project_root, 'data', 'archive', '100k-most-used-passwords-NCSC.txt')
    analyzer = PasswordGeneralAnalysis(data_file)
