...
class SemanticDecompositionAnalyzer:
    ...
    def iter_passwords(self):
        """Iterate through all passwords from target folders"""
        total_scanned = 0
        total_skipped = 0
        
        for folder in TARGET_FOLDERS:
            file_path = self.base_dir / folder / f"{folder}.txt"
        ...
        print(f"\n Scanned {total_scanned} files, skipped {total_skipped}\n")
    ...
    def analyze(self, sample_size: int = None):
        ...
        print(f"Analyzing passwords from: {', '.join(TARGET_FOLDERS)}\n")
        ...
        for idx, password in enumerate(self.iter_passwords()):
            ...
            # Pattern type classification (use actual dictionary hits, not just extracted tokens)
            matched_tokens = plain_hits.union(leet_hits) if (plain_hits or leet_hits) else set()
            pattern_type = self._classify_pattern_type(password, matched_tokens)
    ...
    def _classify_pattern_type(self, password: str, matched_dict_tokens: Set[str]) -> str:
        """Classify password pattern type. matched_dict_tokens = set of words that matched the dictionary."""
        has_dict_word = bool(matched_dict_tokens)
    ...
    def _save_markdown_report(self, output_dir: Path):
        """Save detailed markdown report with all key information"""
        report_file = output_dir / "STEP4_SEMANTIC_REPORT.md"
        total = self.results['total_passwords']

        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("# Step 4: Semantic Decomposition Report (0–D)\n\n")
            f.write(f"**Data source:** folders `{', '.join(TARGET_FOLDERS)}` under base archive.\n\n")
            ...
            if total > 0:
                ...
                f.write(f"({self.results['dictionary_matches']/total*100:.2f}%)\n")
                f.write(f"- **L33t Speak Detected**: {self.results['leet_matches']:,} ")
                f.write(f"({self.results['leet_matches']/total*100:.2f}%)\n")
                f.write(f"- **Keyboard Patterns**: {self.results['keyboard_patterns']:,} ")
                f.write(f"({self.results['keyboard_patterns']/total*100:.2f}%)\n\n")
            else:
                f.write("- **Dictionary Matches**: No passwords analyzed\n")
                f.write("- **L33t Speak Detected**: No passwords analyzed\n")
                f.write("- **Keyboard Patterns**: No passwords analyzed\n\n")

            f.write("## Top 20 Matched Dictionary Words\n\n")
            f.write("| Rank | Word | Count |\n")
            f.write("|------|------|-------|\n")
            for i, (word, count) in enumerate(list(self.results['matched_words'].items())[:20], 1):
                f.write(f"| {i} | {word} | {count:,} |\n")
            f.write("\n")

            f.write("## L33t Speak Examples\n\n")
            if self.results['leet_examples']:
                for ex in self.results['leet_examples'][:15]:
                    f.write(f"- `{ex['password']}` → words: {', '.join(ex['leet_words'])}\n")
            else:
                f.write("None captured.\n")
            f.write("\n")

            f.write("## Keyboard Pattern Examples\n\n")
            if self.results['keyboard_examples']:
                for pwd in self.results['keyboard_examples'][:15]:
                    f.write(f"- `{pwd}`\n")
            else:
                f.write("None captured.\n")
            f.write("\n")

            f.write("## Pattern Type Ranking (Most → Least Predictable)\n\n")
            pattern_types = sorted(
                self.results['pattern_types'].items(),
                key=lambda x: x[1],
                reverse=True
            )
            pattern_total = sum(v for _, v in pattern_types)
            f.write("| Rank | Pattern Type | Count | % |\n")
            f.write("|------|---------------|-------|-----|\n")
            for rank, (pattern_type, count) in enumerate(pattern_types, 1):
                pct = (count / pattern_total * 100) if pattern_total else 0
                f.write(f"| {rank} | {pattern_type} | {count:,} | {pct:.2f}% |\n")
            f.write("\n")
            ...
            for pos, count in sorted(self.results['word_pos_tags'].items(), key=lambda x: x[1], reverse=True):
                ...
                for word, word_count in list(top_words.items())[:10]:
                    definition = ""
                    if pos in self.results['wordnet_examples'] and word in self.results['wordnet_examples'][pos]:
                        definition = f" — *{self.results['wordnet_examples'][pos][word]}*"
...
def parse_args():
    # Default output to the 0-D folder (same directory as this script)
    ...
    parser.add_argument(
        ...
        help="Base directory containing password files",
    )
    parser.add_argument(
        "--sample",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Output directory for results (default: 0-D folder)",
    )
...
def main():
    ...
    analyzer = SemanticDecompositionAnalyzer(base_dir)
