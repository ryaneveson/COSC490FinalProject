...
def build_ngram_counts(base_dir: Path, folders, verbose=False, progress_interval=100000):
    unigram = Counter()
    ...
    for folder in folders:
        ...
        with path.open("r", encoding="utf-8", errors="replace") as fd:
            for i, raw_line in enumerate(fd, start=1):
                ...
                if pbar:
                    pbar.update(1)
                elif verbose and folder_passwords % progress_interval == 0:
                    print(f"  Folder {folder}: {folder_passwords:,} passwords processed (total {total_passwords:,})")

        if verbose:
            print(f"  done folder {folder}: {folder_passwords:,} passwords processed; total {total_passwords:,}")
...
def compute_full_stats(base_dir: Path, folders, counts, verbose=False, progress_interval=100000):
    ...
    # Gather all passwords for parallel scoring
    all_passwords = []
    pwd_lengths = []
    for folder in folders:
        file_name = f"{folder}.txt" if folder != "Symbols" else "Symbols.txt"
        ...
        with path.open("r", encoding="utf-8", errors="replace") as fd:
            for raw_line in fd:
                ...
                if not line:
                    continue
                all_passwords.append(line)
                pwd_lengths.append(len(line))
...
Data source: folders `{', '.join(target_folders)}` under base archive.
...
def password_score(password: str, counts, smoothing=1e-6):
    ...
    # Less strict scaling: more passwords get higher base scores
    predictability = max(0, min(100, -avg_log_prob * 2))
...
def main():
    ...
    counts = build_ngram_counts(base_dir, TARGET_FOLDERS, verbose=args.verbose, progress_interval=args.progress_interval)

    # Compute full stats
    if args.verbose:
        print("Computing predictability scores...")
    stats = compute_full_stats(base_dir, TARGET_FOLDERS, counts, verbose=args.verbose, progress_interval=args.progress_interval)
    ...
    report_md = generate_report(counts, stats, TARGET_FOLDERS)
